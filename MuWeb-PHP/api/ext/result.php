					...
					//	if ($data['buyerPayAmount'] != $data['totalAmount']) throw new Exception("付款金额与物品金额不符。");
						...
						for($i=0;$i<count($cart);$i++){
							...
							if ($config['sendType']==2) {
								...
								$newgminventory =substr($newgminventory,0,3840);
							...
							$web->query("UPDATE [X_TEAM_MARKET_ITEM] SET [status] = ?,[buy_username] = ?,[buy_price] = ?,[buy_alipay] = ?,[pay_out_trade_no] = ?,[buy_date] = ? WHERE [ID] = ?", [2, $_SESSION['username'], $price[$i], $buyerUserId, $PayNo, logDate(), $noData[$i]['ID']]);
					   ...
					   #所有都完成以后给卖家付款
						$SellAccountID = $web->query_fetch_single("SELECT [alipay] FROM [" . X_TEAM_ACCOUNT . "] WHERE [servercode] = ? AND [Account] = ?", [getServerCodeForGroupID($_SESSION['group']), $noData[0]['username']]);
					 ...
					 //	exit(json_encode(["code" => "10000", "msg" => "Success", "data" =>  "恭喜您，购买成功！<br>".$itemName."道具已发放至您的仓库中"]));
					   exit(jsAlt("恭喜您，购买成功！<br>".$itemName."道具已发放至您的". $target."中", 'success'));
