    ...
    case "/market/character":
        ...
        switch ($_POST['modules']){
            case "qrcocde":
                ...
                #申请二维码
                $url = $trading->preCreate("[九鼎奇迹]角色购买", $trade_no, $getData[0]['price']);
                if($url['code'] !== "10000") exit(json_encode(["code" => $url['code'], "msg" => $url['msg']]));
                $content = '';
                    $content .='<div class="align-self-center mt-1 mb-1 text-center text-white">';
                    $content .='<div style="font-size:20px">使用支付宝[扫一扫]</div>';
                    $content .= $trading->qrCode($url['url']);
                    $content .='<div class="qrcode-time-content">您有<span id="qrcode-time">2</span>分钟来完成该笔订单</div>';
                $content .= '</div>';
...
//                break;
        ...
        foreach ($list as $item) {
            #不显示自己寄售的角色
            $myCard = 0;
            if(isLoggedIn() && $_SESSION['group'] == $item['group'] && $_SESSION['username'] == $item['username']) $myCard = 1;
            $data[] = [
                "no" => $i,
                "uid" => $item['ID'],
                ...
                "servercode" => getGroupNameForServerCode($item['group']),
                ...
                case "qrcocde":
                    ...
                    if(is_array($_POST['cart'])){
                        ...
                        for ($i = 0;$i<count($_POST['cart']);$i++){
                            ...
                            #申请订单号
                            if(!$market->getMarketOutTradeNo($_POST['cart'][$i],$_SESSION['username'],$trade_no)) exit(json_encode(["code" => "20005", "msg" => "订单号申请失败，请稍后尝试或联系在线客服。"]));
                    ...
                    #申请二维码
                    $url = $trading->preCreate("[九鼎奇迹]物品购买", $trade_no, $price);
                    if($url['code'] !== "10000") exit(json_encode(["code" => $url['code'], "msg" => $url['msg']]));
                    $content = '';
                    $content .='<div class="align-self-center mt-1 mb-1 text-center text-white">';
                    $content .='<div style="font-size:20px">使用支付宝[扫一扫]</div>';
