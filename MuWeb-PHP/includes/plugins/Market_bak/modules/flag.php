...
try {
    ...
    if(!$moduleConfig['active']) throw new Exception('未开放旗帜交易市场！');
    ...
    if(check_value($sName)) $list = getArrayKeyForValue($sName,$list,'name');
    #分页需求
    ...
    ?>
    <div class="card mb-3">
        <div class="card-header">旗帜市场</div>
        <form action="">
            <div class="mt-3">
                <div class="form-group row justify-content-md-center">
                    ...
                    <div class="col-sm-3" style="padding-left: 5px;padding-right: 5px;">
                        <input type="text" class="form-control" name="name" placeholder="卖家名称" />
                    </div>
                    ...
                    <tr class="data-info" data-info="<?=$data['item_code'];?>">
                        ...
                        <td>
                            <?if(isset($_SESSION['username']) && isset($_SESSION['group']) && $data['username'] == $_SESSION['username'] && $data['servercode'] == getServerCodeForGroupID($_SESSION['group'])){?>
                                <a href="<?=__BASE_URL__?>Market/flag/<?=$data['ID']?>" class="btn btn-danger col-md-8">下架</a>
                            <?}else{?>
