...
try {
    ...
    $modeConfig = $market->loadConfig('char');
    if(!$modeConfig['active']) throw new Exception('未开放角色交易市场！');
    $market->MarketMenu();
    $common = new common();
        ...
        <div class="">
            <table class="table table-striped table-bordered text-center">
                ...
                foreach($AccountCharacters as $code=>$thisCharacter) {
                    ...
                    if($modeConfig['min_level'] > $countLevel || $modeConfig['max_level'] < $countLevel) continue;
                    ?>
                    <form class="form-horizontal" action="" method="post">
                        <tr>
                            ...
                            <td>
                                ...
                                <button name="submit" value="submit" class="btn btn-warning col-md-12">售卖</button>
                            </td>
                        </tr>
                    </form>
                <?}?>
            ...
            <table class="table table-striped table-bordered text-center table-sm">
                ...
                <?foreach ($MyList as $data){?>
                    <tr>
                        ...
                        <td><?=$data['date']?></td>
                        <td><a href="<?=__BASE_URL__?>Market/CharacterSell/<?=$data['ID']?>" class="btn btn-outline-success btn-sm col-md-8">角色下架</a></td>
                    </tr>
                <?}?>
                </tbody>
            </table>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <p class="alert alert-info">1、交易限制：每个角色仅可寄售<span class="text-danger"><?=$modeConfig['frequency']?></span>次，未成功交易过不算在内！</p>
            <p class="alert alert-info">2、等级限制：寄售角色仅显示等级+大师等级<span class="text-danger"><?=$modeConfig['min_level']?>~<?=$modeConfig['max_level']?></span>级之间！</p>
