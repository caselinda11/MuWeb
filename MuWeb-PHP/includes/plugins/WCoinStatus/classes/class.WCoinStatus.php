...
class WCoinStatus {
    ...
    public function setWCoinStatus($group, $username, $char_name, $point)
    {
        ...
        if(!Token::checkToken('WCoinStatus')) throw new Exception('出错了，请您重新输入！');
        ...
        if($data['Point1'] < $this->config['points_1']){
            ...
            $creditType = $this->config['credit_type_1'];
            $price = $this->config['credit_price_1'] * $point;
        }
            ...
            $creditType = $this->config['credit_type_2'];
            $price = $this->config['credit_price_2'] * $point;
        }
            ...
            $creditType = $this->config['credit_type_3'];
            $price = $this->config['credit_price_3'] * $point;
        }
            ...
            $creditType = $this->config['credit_type_4'];
            $price = $this->config['credit_price_4'] * $point;
        }
            ...
            $creditType = $this->config['credit_type_5'];
            $price = $this->config['credit_price_5'] * $point;
        }else{
        ...
        $this->creditSystem->setConfigId($creditType);
        ...
        try {
            ...
            $this->creditSystem->subtractCredits($group,$price);

            $muOnline->query("update Character set LevelUpPoint = LevelUpPoint+".$point.",".$query." WHERE [AccountID] = ? AND [Name] = ?",[$username,$char_name]);

            #写入日志
            if($this->config['log']) {
                $this->Web->query("INSERT INTO [X_TEAM_WCOINSTATUS_LOG] ([AccountID],[Servercode],[Name],[Stage],[Credit],[Credit_type],[Date]) VALUES (?,?,?,?,?,?,?)",[$username,getServerCodeForGroupID($group),$char_name,$Stage,$price,$creditType,logDate()]);
            }

            $muOnline->commit();
            $this->Web->commit();
            alert('usercp/WCoinStatus','恭喜您角色铸造成功，点击确定将返回主界面！');
	...
	public function loadModule($module) {
		...
		if(!@include_once(__PATH_WCOIN_STATUS_ROOT__.$this->_modulesPath.'/'.$module.'.php')) throw new Exception('无法加载角色铸造插件模块。');
	}
	...
	private function _moduleExists($module) {
		...
		if(!file_exists(__PATH_WCOIN_STATUS_ROOT__ . $this->_modulesPath . '/' . $module . '.php')) return;
		return true;
	}
    ...
    {
        $xmlPath = __PATH_WCOIN_STATUS_ROOT__.$file.'.xml';
