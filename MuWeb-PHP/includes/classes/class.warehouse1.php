...
class warehouse1 {
    ...
    private $WAREHOUSE_LENGTH = 3840;   //有4个Inventory;  1920    igcn = 8个仓库  
    private $WAREHOUSE_MAX_SIZE = 240;
    ...
    public function __construct($group, $username = '')
    {
		//version
		//
		#$this->useExtendWarehouse=useExtendWarehouse;
        #服务器类型
        global $config;
        ...
        #初始化
        $this->_width = 8;
        $this->_height = ($this->useExtendWarehouse) ? 30: 15;
    ...
    public function warehouseAddItem($itemCode){
        ...
        if(!$newWarehouse) throw new Exception("[WAREHOUSE] 仓库位置不足，请确保仓库上分8*8位置有足够的位置存放。");
        #拼接
...
//        if(!stripos($newWarehouse,$itemCode)) throw new Exception("[WAREHOUSE] 操作失败,物品错误!");
    ...
    public function getWarehouse($group, $username)
    {
        ...
        $result = Connection::Database("MuOnline",$group)->query_fetch_single("SELECT CONVERT(varchar(max),Items,2) as Items FROM warehouse WHERE AccountID = ?",[$username]);
        if (!$result) throw new Exception('您还没打开过仓库,请先进游戏打开一次仓库再来!');
        #预防无数据
//        if(empty($result)) $result['Items'] = str_pad("F",$this->WAREHOUSE_LENGTH,"F");
    ...
    public function insertWarehouse($group,$username)
    {
        ...
        if(!check_value($username)) throw new Exception('[WAREHOUSE] 请求错误！');
        if(!Validator::AlphaNumeric($username)) throw new Exception('[WAREHOUSE] 账号错误哦，获取失败！');
        $result = $this->muonline->query("INSERT INTO [warehouse]  ([AccountID],[Items]) VALUES (?,CONVERT(VARBINARY(MAX), ? ,2))",[$username,$this->NULLWAREHOUSE]);
    ...
    {
        if($this->_width < $Width) throw new Exception("操作失败,该物品宽度大于仓库宽度!");
        if($this->_height < $height) throw new Exception("操作失败,该物品高度大于仓库高度!");
    ...
    public function resetWarehouse($group, $username, $password)
    {
        ...
        $NULLWAREHOUSE = str_pad("F", 3840,"F");
        $result = $this->muonline->query("UPDATE [warehouse] SET [Items] = CONVERT(VARBINARY(MAX),?,2) WHERE [AccountID] = ?",[$NULLWAREHOUSE,$username]);
        if(!$result) throw new Exception('[WAREHOUSE-ERROR]操作失败，请联系客服。');
        return alert('usercp/ResetWarehouse','您的仓库已重置成功！');
    }
