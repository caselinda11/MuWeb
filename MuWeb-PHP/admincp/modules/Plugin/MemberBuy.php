        ...
        <div class="page-title-box">
            <div class="btn-group float-right">
                <ol class="breadcrumb hide-phone p-0 m-0">
                    ...
                    <li class="breadcrumb-item active">会员升级</li>
                </ol>
            </div>
            <h4 class="page-title">会员升级</h4>
        </div>
    </div>
</div>
<?php
try {
    ...
    function submit()
    {
        $xmlPath = __PATH_MEMBER_BUY_ROOT__ .'config.xml';
        ...
        switch ($_POST['submit']){
            case "submit_add":
                try{
                    ...
                    if(!check_value($_POST['price'])) throw new  Exception("添加失败，请先填写会员价格。");
                    if(!check_value($_POST['price_type'])) throw new  Exception("添加失败，请先填写价格类型。");
                    ...
                    $sql = $web->query("INSERT INTO [X_TEAM_MEMBER_BUY]([vip_name],[vip_code],[Description],[price],[price_type],[status]) VALUES (?,?,?,?,?,?)",$query);
            ...
            case "submit_edit":
                try{
                    ...
                    if(!check_value($_POST['vip_code'])) throw new  Exception("添加失败，请先填写会员编号。");
                    if(!check_value($_POST['price'])) throw new  Exception("添加失败，请先填写会员价格。");
                    if(!check_value($_POST['price_type'])) throw new  Exception("添加失败，请先填写会员价格。");
                    if(!check_value($_POST['status'])) throw new  Exception("添加失败，请确保数据的正确性！");
                    if(!check_value($_POST['Description'])) throw new  Exception("添加失败，请先填写会员描述。");
                    $query = [
                    ...
                    $sql = $web->query("UPDATE [X_TEAM_MEMBER_BUY] SET [vip_name] = :vip_name,[vip_code] = :vip_code,[Description] = :Description,[price] = :price,[price_type] = :price_type,[status] = :status WHERE [ID] = :ID",$query);
            ...
            case "submit_delete":
                try{
                    if(!check_value($_POST['ID'])) throw new  Exception("操作失败，请确保数据的正确性！");
                    $delete = $web->query("DELETE FROM [X_TEAM_MEMBER_BUY] WHERE [ID] = ?",[$_POST['ID']]);
...
try {
    $MemberList = new \Plugin\MemberBuy();
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tr>
                        <td><strong>日志开关</strong>  <span
                                class="text-muted">启用/禁用 日志是否写入数据库 <strong>日志表:[X_TEAM_MEMBER_BUY_LOG]</strong></td>
    ...
    <div class="card">
        ...
        <div>
            <table class="table table-striped table-bordered text-center">
                ...
                <tbody>
                <?$list = $MemberList->getMemberBuyList()?>
                <?IF(is_array($list)){?>
