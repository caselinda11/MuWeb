...
#读取侧边栏配置文�?
$sidebarConfig = gconfig('sidebar');     //加载侧边栏配置文�?
?>
        ...
        if(is_array($srvInfoCache)) {?>
            <div class="card mb-3">
                ...
                <table class="table table-hover table-striped text-center" style="margin-bottom: 0">
                    <tr><td width="50%">总账�?/td><td style="font-weight:bold;"><?=number_format($srvInfoCache[0][0]*1);?></td></tr>
                    <tr><td>总角�?/td><td style="font-weight:bold;"><?=number_format($srvInfoCache[0][1]*1);?></td></tr>
                    <tr><td>总战�?/td><td style="font-weight:bold;"><?=number_format($srvInfoCache[0][2]*1);?></td></tr>
                    <tr><td>总在�?/td><td style="color:#00aa00;font-weight:bold;"><?=number_format($srvInfoCache[0][3]*1);?></td></tr>
                    <?
                    $onlineDays = 1; // 边栏服务器在线天数开�?
                    if($onlineDays){//如果$onlineDays为真
                        ...
                        ?>
                        <tr><td colspan="2" style="text-align:center;"><?=config('server_name')?>已稳定运�?font style="color:#00aa00;font-weight:bold;"> <?=$sod['onlineDays'];?> </font>�?/td></tr>
                    <?}?>
                </table>
            </div>
        <?}
...
if($sidebarConfig['qqun_link_sidebar']){?>
    <div class="mb-3">
        <a href="<?=config('QQUN')?>" class="btn-social-counter qqQun" target="_blank">
            ...
            <h6 class="btn-social-counter__title">加入玩家交流�?/h6>
            <span class="btn-social-counter__count">
                <span class="btn-social-counter__count-num"></span>
                 加入玩家交流群与我们一起互�?
            </span>
        ...
        if (!empty($ranking_data)){?>
            <?if(is_array($ranking_data['castle'])) {?>
                <div class="card mb-3">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        罗兰信息
                        <a href="<?=__BASE_URL__;?>castlesiege" class="more" title="查看更多">&nbsp;�?nbsp;</a>
                    </div>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-7 text-center">
                                                    <table class="table table-sm text-center">
                                                        ...
                                                        <tr><td colspan="2">下一次攻城时�?/td></tr>
                                                    </table>
                                                    <div id="csCountDown">
                                                        <?if($cs > 86400) echo $timeLeft[0];?>�?
                                                        <?if($cs > 3600) echo $timeLeft[1];?>�?
                                                        <?if($cs > 60) echo $timeLeft[2];?>�?
                                                        <?=$timeLeft[3]?>�?
                                                    </div>
                                                </div>
                                            </div>
...
<? try{
    if($sidebarConfig['bfk_sidebar']) {
        ...
        if(is_array($BFB)){
            ?>
            <div class="card mb-3">
                <div class="card-header">冰风谷城主信�?/div>
                <div class="card-body">
...
<? try {
    ...
    if(!empty($levelRankingData)) {
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                角色排名
                <a href="<?=__BASE_URL__;?>rankings/level" class="more" title="查看更多">&nbsp;�?nbsp;</a>
            </div>
    ...
    if($sidebarConfig['gens_ranking_sidebar']){
        ...
        if(is_array($ranking_data)) {
            ...
            <div class="card mb-3">
                <div class="card-header d-flex justify-content-between align-items-center">
                    家族排名
                    <a href="<?=__BASE_URL__;?>rankings/gens" class="more" title="查看更多">&nbsp;�?nbsp;</a>
                </div>
                <table class="table table-hover table-striped text-center">
                    <tr>
                        <th>角色</th>
